        ...
        {visibleItems.map((item) => {
          ...
          return (
            <div key={item.id} className="w-full">
              {hasChildren ? (
                ...
                >
                  <div className="w-[3rem] h-auto text-start">
                    <FontAwesomeIcon
                      icon={item.icon}
                      className="text-inherit text-[2.5rem] lg:text-[1.6rem]"
                    />
                  </div>
              ...
              {hasChildren && (
                <div
                  className={`overflow-hidden lg:pl-[4.8rem] transition-all duration-500
                  ${isOpen ? "opacity-100 max-h-[30rem]" : "max-h-0 opacity-0"}`}
                >
                  {/* Thực đơn: category động từ API — dùng cat.name (đúng schema Prisma) */}
                  {item.id === 2
                    ? children.map((cat) => (
                        <Link
